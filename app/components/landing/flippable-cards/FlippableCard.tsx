...
export default function FlippableCard({ data }: { data: CardData }) {
    ...
    return (
        <div className="group md:h-96 md:w-96 h-80 w-80">
            ...
            >
                {/* Front Face */}
                <div className="absolute inset-0 h-full w-full rounded-xl bg-[#4076AC] [backface-visibility:hidden] duration-300">
                    <div className="flex flex-col items-center justify-center h-full md:p-6">
                        <Icon className="w-16 h-16 mb-4 text-green-400" />
                        <h1 className="text-2xl text-muted font-bold mb-2">{data.title}</h1>
                ...
                {/* Back Face */}
                <div className="absolute flex flex-col items-center justify-evenly p-2 inset-0 h-full w-full rounded-xl bg-[#4076AC] dark:bg-[#0F0F11] [transform:rotateY(180deg)] [backface-visibility:hidden] overflow-hidden duration-300">
                    <div className="flex flex-col max-w-xs items-center">
                        <p className="text-left font-medium text-muted leading-2">{data.description}</p>
                    </div>
                    <Button asChild size="lg" className="text-lg inline-flex ">
